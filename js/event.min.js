...
self.eventModel.hasChanged("description")||self.eventModel.hasChanged("location"))self.headerView.render();if(self.eventModel.hasChanged("last_update")){self.headerView.reset(self.eventModel.get("last_update"));self.eventModel.setSinceRequestURL()}if(self.eventModel.hasChanged("last_update")||self.eventModel.hasChanged("histogram")){console.log("HOW AOBUT THIS?");self.temporalModel.set({startDate:self.eventModel.get("startDate"),endDate:self.eventModel.get("last_update"),startRange:self.eventModel.get("startDate"),
endRange:self.eventModel.get("last_update"),min:Math.min(self.temporalModel.get("min"),self.eventModel.get("histogram").min),max:Math.max(self.temporalModel.get("max"),self.eventModel.get("histogram").max),resolution:self.eventModel.get("histogram").resolution});self.contentController.update();self.sideHistView.render()}if(self.eventModel.hasChanged("location"))self.mapView.render(self.eventModel.get("location").name,self.eventModel.get("location").lat,self.eventModel.get("location").lon)})}};SIVVIT.ItemGroupCollection=
Backbone.Collection.extend({model:SIVVIT.ItemGroupModel,comparator:function(itm){return-itm.get("timestamp")}});SIVVIT.ItemCollection=Backbone.Collection.extend({model:SIVVIT.ItemModel,comparator:function(itm){return-itm.get("timestamp")}});SIVVIT.Lightbox={init:function(){$("#photo-box").fancybox({maxWidth:800,maxHeight:600,fitToView:true,autoSize:true,width:"70%",height:"70%",closeClick:false,transitionIn:"fade",transitionOut:"fade"});$.waypoints.settings.scrollThrottle=30;$("#wrapper").waypoint.defaults.onlyOnScroll=
true;$("#wrapper").waypoint({offset:"-100%"}).find("#content-stats").waypoint(function(event,direction){$("#mover").toggleClass("sticky",direction==="down");event.stopPropagation()})}};SIVVIT.Parser={parse:function(model){var tmp_group=[];var content=model.get("content"),i,j,tmp_items,group_model,itm_model;var len=content.length;for(i=len;i--;){group_model=new SIVVIT.ItemGroupModel(content[i]);group_model.set({json:model.get("json")});tmp_items=[];for(j=content[i].items.length;j--;){itm_model=new SIVVIT.ItemModel(content[i].items[j]);
itm_model.set({timestamp:content[i].items[j].timestamp});tmp_items.push(itm_model)}group_model.set({id:(new Date).getTime()+"-"+i,items:new SIVVIT.ItemCollection(tmp_items),items_new:new SIVVIT.ItemCollection(tmp_items),stats:content[i].stats,timestamp:content[i].timestamp});model.updateContentRange(group_model.get("timestamp"));tmp_group.push(group_model)}return new SIVVIT.ItemGroupCollection(tmp_group)}};SIVVIT.ContentController=Backbone.View.extend({el:"#navigation-content",prevButton:null,activeButton:null,
view:null,eventModel:null,temporalModel:null,events:{"click #all-btn":"updateView","click #post-btn":"updateView","click #media-btn":"updateView"},initialize:function(options){this.eventModel=options.eventModel;this.temporalModel=options.temporalModel;this.view=options.view;this.activeButton="#all-btn";$(this.activeButton).toggleClass("text-btn",false);$(this.activeButton).toggleClass("text-btn-selected",true)},update:function(){this.renderStats();this.renderHistogram()},updateView:function(event){if(this.renderButtons(event.target.id)){this.update();
