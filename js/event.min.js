...
1},{silent:true})}},this)},updateGroup:function(group){var tmp=[],i,len,items;var content=group.get("content");len=content.length;for(i=len;i--;)if((new Date(content[i].timestamp)).getTime()===group.get("timestamp").getTime())items=content[i].items;if(items.length>0){len=group.get("items").length;for(i=len;i--;){var itm=items[i];if(itm){var itm_model=new SIVVIT.ItemModel(itm);itm_model.set({timestamp:new Date(itm.timestamp)});tmp.push(itm_model);group.get("old_items").add(itm_model)}}group.set({items:group.get("old_items"),
...
itm.html.find("#load-itm").show()},deleteItem:function(itm){this.disableItem(itm);var self=this;itm.model.set({status:-1});itm.model.save({success:function(){itm.html.fadeOut();self.model.remove(itm.model,{silent:true})},error:function(){self.enableItem(itm);self.showHidePending(itm)}})},approveItem:function(itm,value){this.disableItem(itm);var self=this;if(value===undefined)value=Number(itm.model.get("status"))===1?0:1;else value=value===true?1:0;itm.model.set({status:value});itm.model.save({success:function(){self.enableItem(itm);
self.showHidePending(itm)},error:function(){self.enableItem(itm);self.showHidePending(itm)}})},showHidePending:function(itm){if(Number(itm.model.get("status"))===1){itm.html.find("#pending-flag").toggleClass("pending-notice",false);itm.html.find("#pending-flag").toggleClass("active-notice",true)}else{itm.html.find("#pending-flag").toggleClass("pending-notice",true);itm.html.find("#pending-flag").toggleClass("active-notice",false)}},getItemCount:function(group){return group.get("stats").total}});SIVVIT.MapView=
Backbone.View.extend({el:"#map-container",render:function(name,lon,lat){$(this.el).html('<img src="http://maps.googleapis.com/maps/api/staticmap?center='+lon+","+lat+'&zoom=10&size=280x130&sensor=false">')}});SIVVIT.HeaderView=Backbone.View.extend({timestamp:null,render:function(){$("#event-title").html(this.model.get("title"));$("#event-description").html(this.model.get("description"));$("#event-user").html("<span class='gray-text'>Created by</span> <span class='icon-user'></span><a href='#'>"+this.model.get("author")+
"</a> <span class='gray-text'>on</span> "+(new Date(this.model.get("startDate"))).toDateString());$("#map-label").html("<span class='icon-location'></span>"+this.model.get("location").name)},reset:function(date){var self=this;this.timestamp=date;this.update()},update:function(){if(this.model.get("status")>0)$("#timeline-label").html("<span class='icon-time'></span>Live, "+this.formatTime(new Date-this.timestamp));else $("#timeline-label").html("<span class='icon-time'></span>This event archived.")},
