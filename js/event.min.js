...
"items_new":"items").each(function(itm){itm=this.buildTemplate(itm);if(itm){this.initItem(itm,group);group.set({displayed:dsp+=1},{silent:true})}},this)},updateGroup:function(group){var tmp=[],i,len,items;var content=group.get("content");len=content.length;for(i=len;i--;)if((new Date(content[i].timestamp)).getTime()===group.get("timestamp").getTime())items=content[i].items;if(items.length>0){len=group.get("items").length;for(i=len;i--;){var itm=items[i];if(itm){var itm_model=new SIVVIT.ItemModel(itm);
