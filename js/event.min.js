...
$("#event-user").html("<span class='gray-text'>Created by</span> <span class='icon-user'></span><a href='#'>"+this.model.get("author")+"</a> <span class='gray-text'>on</span> "+(new Date(this.model.get("startDate")*1E3)).toDateString());$("#map-label").html("<span class='icon-location'></span>"+this.model.get("location").name)},reset:function(date){var self=this;this.timestamp=date;this.update()},update:function(){switch(this.model.get("status")){case 1:$("#timeline-label").html("<span class='icon-time'></span>Live, "+
...
delete copy.content;delete copy.last_update;delete copy.pending;delete copy.stats;delete copy.histogram;delete copy.meta;delete copy.bucket_limit;delete copy.bucket_page;delete copy.userid;delete copy.content_bounds;$.ajax({url:"http://sivvit.com/e/event/"+this.get("id"),data:copy,type:"PUT",dataType:"json"})},appendHistogram:function(hash,value){var len=value.length;var result=[];for(var i=len;i--;)if(hash[value[i].timestamp])hash[value[i].timestamp].count=Number(hash[value[i].timestamp].count)+
Number(value[i].count);else hash[value[i].timestamp]=value[i];for(var bucket in hash)if(hash[bucket])result.push(hash[bucket]);return result},updateContentRange:function(date){if(this.attributes.content_bounds.min===null){this.attributes.content_bounds.min=new Date(this.get("endDate"));this.attributes.content_bounds.max=new Date(this.get("startDate"))}this.attributes.content_bounds.min=Math.min(date,this.attributes.content_bounds.min);this.attributes.content_bounds.max=Math.max(date,this.attributes.content_bounds.max)},
hasMoreContent:function(){if(this.get("content_bounds").min>new Date(this.get("startDate")))return true;else return false},setSinceRequestURL:function(){var path=this.attributes.json;if(this.attributes.meta!==null)path+="&meta="+this.attributes.meta;if(this.attributes.limit!==null)path+="&limit="+this.attributes.limit;if(this.attributes.last_update!==null)path+="&since="+this.attributes.last_update;if(this.attributes.bucket_limit!==null)path+="&bucket_limit="+this.attributes.bucket_limit;if(this.attributes.bucket_page!==
