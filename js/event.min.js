...
"s<span class='icon-download'></span></div>");btn=$(this.el).find("#load-groups-btn");btn.waypoint(function(event,direction){btn.waypoint("remove");btn.html("<span class='loader'>&nbsp;</span>");self.display_buckets=true;self.eventModel.loadMoreContent()},{offset:"100%",onlyOnScroll:true})}},showLoader:function(show){$(this.el).empty();$(this.el).html("<div id='content-loader'></div>")},buildTemplate:function(itm){var html;switch(itm.get("type")){case "photo":html=$.tmpl(this.photo_template,{thumbnail:itm.get("thumbnail"),
