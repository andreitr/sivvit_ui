...
itm.html.find("#load-itm").show()},deleteItem:function(itm){this.disableItem(itm);var self=this;itm.model.set({status:-1});itm.model.save({type:"DELETE",complete:function(response){if(response.status===200){itm.html.fadeOut();self.model.remove(itm.model,{silent:true})}else{self.enableItem(itm);self.showHidePending(itm)}}})},approveItem:function(itm,value){this.disableItem(itm);var self=this;if(value===undefined)value=Number(itm.model.get("status"))===1?0:1;else value=value===true?1:0;itm.model.set({status:value});
itm.model.save({type:"PUT",success:function(){self.enableItem(itm);self.showHidePending(itm)},error:function(){self.enableItem(itm);self.showHidePending(itm)}})},showHidePending:function(itm){if(Number(itm.model.get("status"))===1){itm.html.find("#pending-flag").toggleClass("pending-notice",false);itm.html.find("#pending-flag").toggleClass("active-notice",true)}else{itm.html.find("#pending-flag").toggleClass("pending-notice",true);itm.html.find("#pending-flag").toggleClass("active-notice",false)}},
...
this.model.get("author")+"</a> <span class='gray-text'>on</span> "+(new Date(this.model.get("startDate"))).toDateString());$("#map-label").html("<span class='icon-location'></span>"+this.model.get("location").name)},reset:function(date){var self=this;this.timestamp=date;this.update()},update:function(){switch(this.model.get("status")){case 1:$("#timeline-label").html("<span class='icon-time'></span>Live, "+this.formatTime(new Date-this.timestamp));break;case 2:$("#timeline-label").html("<span class='icon-time'></span>Archived event.");
break;case -1:$("#timeline-label").html("<span class='icon-time'></span>Stopping event tracking...");break;case 0:$("#timeline-label").html("<span class='icon-time'></span>Initializing event tracking...");break}},formatTime:function(milliseconds){var seconds=Math.floor(milliseconds/1E3);var minutes=Math.floor(milliseconds/6E4);var hours=Math.floor(milliseconds/36E5);var days=Math.floor(milliseconds/864E5);if(days>0)return"updated "+days+" days ago";if(hours>0)return"updated "+hours+" hrs ago";if(minutes>
0)return"updated "+minutes+" min ago";if(seconds>0)return"updated "+seconds+" sec ago";return"updated just now"}})})(jQuery,SIVVIT);SIVVIT.EventModel=Backbone.Model.extend({defaults:{json:null,meta:1,limit:3,bucket_limit:5,bucket_page:1,pull:true,type:"post",content_bounds:{min:null,max:null},id:null,title:null,author:null,description:null,keywords:[],content:[],location:{lon:null,lat:null,name:null},startDate:new Date,endDate:new Date,status:0,last_update:null,pending:0,stats:{total:0,posts:0,images:0,videos:0},histogram:{min:null,max:null,resolution:null,global:[],media:[],post:[]}},post_hash:{},media_hash:{},global_hash:{},
