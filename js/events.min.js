...
(function(jQuery,SIVVIT){SIVVIT.Events={collection:null,view:null,edit:false,init:function(json){var self=this;this.collection=new SIVVIT.EventsCollection;this.view=new SIVVIT.EventsView({edit:this.edit});$.getJSON(json,function(data){$("#content-loader").remove();$("#event-application").show();var len=data.length,i;for(i=len;i--;){var model=new SIVVIT.EventModel(data[i]);model.set({timestamp:new Date(data[i])});self.collection.add(model)}self.view.model=self.collection;self.view.render()})}};SIVVIT.EventsCollection=
...
edit:false,displayed:false,initialize:function(options){this.edit=options.edit},render:function(){$(this.el).empty();this.displayed=false;this.rendered=[];if(this.edit)this.displayEdit();this.display()},display:function(){$(this.el).append("<ol id='event-list'></ol>");this.model.each(function(itm){itm=this.buildTemplate(itm);console.log(itm.model.get("histogram").global.length);var histogram=(new SIVVIT.HistogramView({el:$(itm.html).find("#histogram"),slider:false,model:new SIVVIT.TemporalModel({startDate:new Date(itm.model.get("startDate")),
endDate:new Date(itm.model.get("endDate")),min:itm.model.get("histogram").min,max:itm.model.get("histogram").max,resolution:itm.model.get("histogram").resolution,histogram:itm.model.get("histogram").global})})).render();this.initItem(itm,"#event-list")},this)},buildTemplate:function(itm){var html=$.tmpl(this.template,{title:itm.get("title"),description:itm.get("description"),posts:itm.get("stats").posts,videos:itm.get("stats").videos,images:itm.get("stats").images,location:itm.get("location").name,
author:itm.get("author")});return{html:html,model:itm}},displayEdit:function(){$(this.el).append('<div id="controls-container"><div id="checkbox"><input type="checkbox" id="group-select"></div><a id="del-all" class="link"><span class="icon-delete"></span>Delete</a><a id="pause-all" class="link"></div>');var self=this;$("#del-all").click(function(){var i=self.rendered.length;var result=[];while(i--){var itm=self.rendered[i];if(itm.html.find("#itm-check").is(":checked"))result.push(itm)}if(result.length>
0)if(confirm("Delete "+result.length+" events?")===true)self.deleteItems(result)});$("#group-select").click(function(){var i=self.rendered.length;var checked=$("#group-select").is(":checked");while(i--){var itm=self.rendered[i];itm.html.find("#itm-check").attr("checked",checked);itm.html.css("background-color",!checked?"#FFFFFF":"#FFFFCC")}})},initItem:function(itm,parent){var self=this;if(itm!==null){if(this.edit){itm.html.find("#content").prepend('<span class="item-edit"><span class="icon-delete" id="del-itm"></span><span class=\'icon-cog\' id=\'edit-itm\'></span><div id="pending-flag"></div></span>');
itm.html.find("#content").prepend('<div id="checkbox"><input type="checkbox" id="itm-check"/></div>');itm.html.find("#del-itm").hide();itm.html.find("#edit-itm").hide();itm.html.find("#edit-itm").fancybox({"transitionIn":"fade","transitionOut":"fade","href":"edit_event.html"});if(itm.model.get("pending")>0)itm.html.find("#title").append("<div id='pending'>pending "+itm.model.get("pending")+"</div>");itm.html.hover(function(event){itm.html.find("#del-itm").show();itm.html.find("#edit-itm").show()},
function(event){itm.html.find("#del-itm").hide();itm.html.find("#edit-itm").hide()});itm.html.click(function(event){var checked;switch(event.target.id){case "del-itm":if(confirm("Delete this event?")===true)self.deleteItem(itm);break;case "edit-itm":break;default:if(itm.html.find("#itm-check").length>0){checked=itm.html.find("#itm-check").is(":checked");itm.html.find("#itm-check").attr("checked",!checked);itm.html.css("background-color",checked?"#FFFFFF":"#FFFFCC")}}event.stopPropagation()});this.toggleLive(itm)}this.rendered.push(itm);
$(parent).append(itm.html)}},deleteItem:function(itm){itm.html.fadeOut();this.model.remove(itm.model,{silent:true})},deleteItems:function(itms){for(var i=itms.length;i--;)this.deleteItem(itms[i])},toggleLive:function(itm){var flag=itm.html.find("#pending-flag");if(itm.model.get("status")===1){flag.toggleClass("idle-notice",false);flag.toggleClass("live-notice",true)}else{flag.toggleClass("idle-notice",true);flag.toggleClass("live-notice",false)}}})})($,SIVVIT);SIVVIT.EventModel=Backbone.Model.extend({defaults:{json:null,meta:1,limit:3,bucket_limit:5,bucket_page:1,type:"post",content_bounds:{min:null,max:null},id:null,title:null,author:null,description:null,keywords:[],content:[],location:{lon:null,lat:null,name:null},startDate:new Date,endDate:new Date,status:0,last_update:null,pending:0,stats:{total:0,posts:0,images:0,videos:0},histogram:{min:null,max:null,resolution:null,global:[],media:[],post:[]}},post_hash:{},media_hash:{},global_hash:{},fetch_interval:null,
