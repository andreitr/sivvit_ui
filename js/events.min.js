...
itm.model.get("id")+'" id=\'edit-itm\'></span><div id="pending-flag"></div></span>');itm.html.find("#content").prepend('<div id="checkbox"><input type="checkbox" id="itm-check"/></div>');itm.html.find("#del-itm").hide();itm.html.find("#edit-itm").hide();if(itm.model.get("pending")>0)itm.html.find("#title").append("<div id='pending'>pending "+itm.model.get("pending")+"</div>");itm.html.hover(function(event){itm.html.find("#del-itm").show();itm.html.find("#edit-itm").show()},function(event){itm.html.find("#del-itm").hide();
itm.html.find("#edit-itm").hide()});itm.html.click(function(event){var checked;switch(event.target.id){case "del-itm":if(confirm("Delete this event?")===true){self.deleteItem(itm);event.stopPropagation()}break;case "edit-itm":break;default:if(itm.html.find("#itm-check").length>0){checked=itm.html.find("#itm-check").is(":checked");itm.html.find("#itm-check").attr("checked",!checked);itm.html.css("background-color",checked?"#FFFFFF":"#FFFFCC")}event.stopPropagation()}});this.toggleLive(itm)}this.rendered.push(itm);
$(parent).append(itm.html)}},initLightbox:function(){$("#edit-itm").fancybox({"width":860,"height":430,"autoScale":true,"scrolling":false,"transitionIn":"fade","transitionOut":"fade","type":"iframe"})},deleteItem:function(itm){itm.html.fadeOut();this.model.remove(itm.model,{silent:true})},deleteItems:function(itms){for(var i=itms.length;i--;)this.deleteItem(itms[i])},toggleLive:function(itm){var flag=itm.html.find("#pending-flag");if(itm.model.get("status")===1){flag.toggleClass("idle-notice",false);
flag.toggleClass("live-notice",true)}else{flag.toggleClass("idle-notice",true);flag.toggleClass("live-notice",false)}}})})($,SIVVIT);SIVVIT.EventModel=Backbone.Model.extend({defaults:{json:null,meta:1,limit:3,bucket_limit:5,bucket_page:1,type:"post",content_bounds:{min:null,max:null},id:null,title:null,author:null,description:null,keywords:[],content:[],location:{lon:null,lat:null,name:null},startDate:new Date,endDate:new Date,status:0,last_update:null,pending:0,stats:{total:0,posts:0,images:0,videos:0},histogram:{min:null,max:null,resolution:null,global:[],media:[],post:[]}},post_hash:{},media_hash:{},global_hash:{},fetch_interval:null,
