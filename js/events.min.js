...
edit:false,displayed:false,initialize:function(options){this.edit=options.edit},render:function(){$(this.el).empty();this.displayed=false;this.rendered=[];if(this.edit)this.displayEdit();this.display()},display:function(){$(this.el).append("<ol id='event-list'></ol>");this.model.each(function(itm){itm=this.buildTemplate(itm);var histogram=(new SIVVIT.HistogramView({el:$(itm.html).find("#histogram"),slider:false,model:new SIVVIT.TemporalModel({startDate:new Date(itm.model.get("startDate")),endDate:new Date(itm.model.get("endDate")),
min:itm.model.get("histogram").min,max:itm.model.get("histogram").max,resolution:itm.model.get("histogram").resolution,histogram:itm.model.get("histogram").post})})).render();this.initItem(itm,"#event-list")},this)},buildTemplate:function(itm){html=$.tmpl(this.template,{title:itm.get("title"),description:itm.get("description"),posts:itm.get("stats").posts,videos:itm.get("stats").videos,images:itm.get("stats").images,location:itm.get("location").name,author:itm.get("author")});return{html:html,model:itm}},
...
this.model.get("startRange").getTime()&&(new Date(bar.timestamp)).getTime()<=this.model.get("endRange").getTime())bar.attr({fill:"#333333"});else bar.attr({fill:"#CCCCCC"})},drawHistogram:function(){if(this.model.get("histogram")){console.log("Rendering view?");var adjusted_end_date=this.model.adjustToNextBucket(new Date(this.model.get("histogramEndDate"))).getTime();var lenTotal=Math.ceil((adjusted_end_date-this.model.get("histogramStartDate"))/this.model.getResolution());var len=this.model.get("histogram").length;
var maxVal=this.model.get("max");var minVal=this.model.get("min");var maxHeight=$(this.el).height();var maxWidth=$(this.el).width();var barW=$(this.el).width()/lenTotal;barW=barW<0.5?0.5:barW;var startTime=this.model.get("histogramStartDate");var endTime=adjusted_end_date;var histogram=Raphael($(this.el)[0],$(this.el).width(),$(this.el).height());for(var i=len;i--;){var frame=this.model.get("histogram")[i];var percentY=frame.count/maxVal*100;var percentX=(frame.timestamp.getTime()-startTime)/(endTime-
startTime);var barH=Math.round(percentY*maxHeight/100);var barX=Math.round(percentX*maxWidth);var barY=Math.round(maxHeight-barH);var bar=histogram.rect(barX,barY,barW,barH).attr({fill:"#333333","stroke-width":0});if(this.slider){bar.timestamp=frame.timestamp;this.updateBarColor(bar);this.bars.push(bar)}}}}});