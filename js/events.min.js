...
0;var tmp_max=0;for(var i=len;i--;){tmp_min=Math.min(tmp_min,attributes.histogram[i].count);tmp_max=Math.max(tmp_max,attributes.histogram[i].count);if(attributes.histogram[i].timestamp instanceof Date===false)attributes.histogram[i].timestamp=Date.secondsToDate(attributes.histogram[i].timestamp);if(this.checkDateBounds(attributes.histogram[i].timestamp)===true){this.bucket_hash[attributes.histogram[i].timestamp]=attributes.histogram[i];if(!this.get("histogramStartDate")||!this.get("histogramEndDate")){this.set({histogramStartDate:attributes.histogram[i].timestamp});
this.set({histogramEndDate:attributes.histogram[i].timestamp})}else{this.set({histogramStartDate:Math.min(attributes.histogram[i].timestamp,this.get("histogramStartDate"))});this.set({histogramEndDate:Math.max(attributes.histogram[i].timestamp,this.get("histogramEndDate"))})}}else attributes.histogram.splice(i,1)}console.log(tmp_max,tmp_min);attributes.min=tmp_min;attributes.max=tmp_max}}Backbone.Model.prototype.set.call(this,attributes,options);return this},adjustResolution:function(date){switch(this.get("resolution")){case "day":return new Date(date.getFullYear(),
