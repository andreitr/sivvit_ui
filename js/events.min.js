...
function(event){itm.html.find("#del-itm").hide();itm.html.find("#edit-itm").hide()});itm.html.click(function(event){var checked;switch(event.target.id){case "del-itm":if(confirm("Delete this event?")===true)self.deleteItem(itm);break;case "edit-itm":console.log(itm.model.get("id"));break;default:if(itm.html.find("#itm-check").length>0){checked=itm.html.find("#itm-check").is(":checked");itm.html.find("#itm-check").attr("checked",!checked);itm.html.css("background-color",checked?"#FFFFFF":"#FFFFCC")}}event.stopPropagation()});
this.toggleLive(itm)}this.rendered.push(itm);$(parent).append(itm.html)}},deleteItem:function(itm){itm.html.fadeOut();this.model.remove(itm.model,{silent:true})},deleteItems:function(itms){for(var i=itms.length;i--;)this.deleteItem(itms[i])},toggleLive:function(itm){var flag=itm.html.find("#pending-flag");if(itm.model.get("status")===1){flag.toggleClass("idle-notice",false);flag.toggleClass("live-notice",true)}else{flag.toggleClass("idle-notice",true);flag.toggleClass("live-notice",false)}}})})($,
