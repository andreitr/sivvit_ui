...
el:"#dynamic-content",models_hash:{},edit:false,displayed:false,initialize:function(options){this.edit=options.edit},render:function(){$(this.el).empty();this.displayed=false;this.display()},display:function(){$(this.el).append("<ol id='event-list'></ol>");this.model.each(function(itm){console.log("--------",itm.get("histogram").global.length);itm=this.buildTemplate(itm);var mdl=new SIVVIT.TemporalModel({startDate:itm.model.get("startDate"),endDate:itm.model.get("last_update"),startRange:itm.model.get("startDate"),
endRange:itm.model.get("last_update"),resolution:itm.model.get("histogram").resolution});mdl.set({"histogram":itm.model.get("histogram").global});var histogram=(new SIVVIT.HistogramView({el:$(itm.html).find("#histogram"),model:mdl})).render();this.initItem(itm,"#event-list");this.models_hash[itm.model.get("id")]=itm},this);this.initLightbox()},buildTemplate:function(itm){var html=$.tmpl(this.template,{link:SIVVIT.Settings.host+"/event/"+itm.get("id"),title:itm.get("title"),description:itm.get("description"),
posts:itm.get("stats").posts,videos:itm.get("stats").videos,images:itm.get("stats").images,location:itm.get("location").name,author:itm.get("author")});return{html:html,model:itm}},initItem:function(itm,parent){var self=this;if(itm!==null){if(this.edit){itm.html.find("#content").prepend('<span class="item-edit"><span class=\'icon-cog\' href="event_form.html?id='+itm.model.get("id")+'" id=\'event-form\'></span><div id="pending-flag"></div></span>');itm.html.find("#event-form").hide();if(itm.model.get("pending")>
0)itm.html.find("#title").append("<div id='pending'>pending "+itm.model.get("pending")+"</div>");itm.html.hover(function(event){itm.html.find("#event-form").show()},function(event){itm.html.find("#event-form").hide()});itm.html.click(function(event){var checked;if(event.target.id!=="event-form"){if(itm.html.find("#itm-check").length>0){checked=itm.html.find("#itm-check").is(":checked");itm.html.find("#itm-check").attr("checked",!checked);itm.html.css("background-color",checked?"#FFFFFF":"#FFFFCC")}event.stopPropagation()}});
this.toggleLive(itm)}$(parent).append(itm.html)}},initLightbox:function(){var self=this;$("#event-form").fancybox({width:860,height:430,autoScale:true,scrolling:false,transitionIn:"fade",transitionOut:"fade",type:"iframe",afterClose:function(){var cookie=JSON.parse($.cookie("com.sivvit.event"));if(cookie)switch(cookie.action){case "delete":if(self.models_hash[cookie.model.id])self.deleteItem(self.models_hash[cookie.model.id]);break;case "update":if(self.models_hash[cookie.model.id]){self.models_hash[cookie.model.id].model.set(cookie.model);
