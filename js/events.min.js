...
(function(jQuery,SIVVIT){SIVVIT.Events={collection:null,view:null,edit:false,init:function(json){var self=this;this.collection=new SIVVIT.EventsCollection;this.view=new SIVVIT.EventsView({edit:this.edit});$.getJSON(json,function(data){$("#content-loader").remove();$("#event-application").show();var len=data.length,i,model;for(i=len;i--;){console.log(data[i]);model=new SIVVIT.EventModel(data[i]);model.set({timestamp:new Date(data[i])});self.collection.add(model)}self.view.model=self.collection;self.view.render()})}};
...
el:"#dynamic-content",rendered:[],edit:false,displayed:false,initialize:function(options){this.edit=options.edit},render:function(){$(this.el).empty();this.displayed=false;this.rendered=[];if(this.edit)this.displayEdit();this.display()},display:function(){$(this.el).append("<ol id='event-list'></ol>");this.model.each(function(itm){itm=this.buildTemplate(itm);var histogram=(new SIVVIT.HistogramView({el:$(itm.html).find("#histogram"),slider:false,model:new SIVVIT.TemporalModel({startDate:new Date(itm.model.get("startDate")),
endDate:new Date(itm.model.get("endDate")),min:itm.model.get("histogram").min,max:itm.model.get("histogram").max,resolution:itm.model.get("histogram").resolution,histogram:itm.model.get("histogram").global})})).render();this.initItem(itm,"#event-list")},this)},buildTemplate:function(itm){html=$.tmpl(this.template,{title:itm.get("title"),description:itm.get("description"),posts:itm.get("stats").posts,videos:itm.get("stats").videos,images:itm.get("stats").images,location:itm.get("location").name,author:itm.get("author")});
return{html:html,model:itm}},displayEdit:function(){$(this.el).append('<div id="controls-container"><div id="checkbox"><input type="checkbox" id="group-select"></div><a id="del-all" class="link"><span class="icon-delete"></span>Delete</a><a id="pause-all" class="link"></div>');var self=this;$("#del-all").click(function(){var i=self.rendered.length;var result=[];while(i--){var itm=self.rendered[i];if(itm.html.find("#itm-check").is(":checked"))result.push(itm)}if(result.length>0)if(confirm("Delete "+
result.length+" events?")===true)self.deleteItems(result)});$("#group-select").click(function(){var i=self.rendered.length;var checked=$("#group-select").is(":checked");while(i--){var itm=self.rendered[i];itm.html.find("#itm-check").attr("checked",checked);itm.html.css("background-color",!checked?"#FFFFFF":"#FFFFCC")}})},initItem:function(itm,parent){var self=this;if(itm!==null){if(this.edit){itm.html.find("#content").prepend('<span class="item-edit"><span class="icon-delete" id="del-itm"></span><span class=\'icon-cog\' id=\'edit-itm\'></span><div id="pending-flag"></div></span>');
