if(typeof SIVVIT=="undefined")SIVVIT={};SIVVIT.Settings={host:"http://sivvit.com"};(function(jQuery,SIVVIT){SIVVIT.Events={collection:null,view:null,edit:true,init:function(json){var self=this;this.collection=new SIVVIT.EventsCollection;this.view=new SIVVIT.EventsView({edit:this.edit});var MyModel=Backbone.Model.extend({url:json});var tmpModel=new MyModel;tmpModel.fetch({success:function(result){$("#content-loader").remove();$("#event-application").show();if(result.attributes){$.each(result.attributes,function(index,value){var actual_model=new SIVVIT.EventModel(value);self.collection.add(actual_model)});
self.view.model=self.collection;self.view.render()}}})}};SIVVIT.EventsCollection=Backbone.Collection.extend({model:SIVVIT.EventModel,comparator:function(itm){return-itm.get("startDate")}});SIVVIT.EventsView=Backbone.View.extend({template:"<li id='post-list'><div id='content'><div id='histogram'></div><div id='title'><a href='${link}'>${title}</a></div><div id='description'>${description}</div><div id='meta'>${posts} posts, ${images} images, ${videos} videos &nbsp; &nbsp;<span class='icon-location'></span>${location} &nbsp;<span class='icon-user'></span><a href='#'>${author}</a></div></div></div></li>",
