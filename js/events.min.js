...
this;$("#event-form").fancybox({width:860,height:430,autoScale:true,scrolling:false,transitionIn:"fade",transitionOut:"fade",type:"iframe",afterClose:function(){var cookie=JSON.parse($.cookie("com.sivvit.event"));if(cookie){console.log("WE HAVE A COOKIE!!!!!",cookie.action);switch(cookie.action){case "delete":if(self.models_hash[cookie.model.id])self.deleteItem(self.models_hash[cookie.model.id]);break;case "update":if(self.models_hash[cookie.model.id]){self.models_hash[cookie.model.id].model.set(cookie.model);
self.updateItem(self.models_hash[cookie.model.id])}break;case "create":var new_model=new SIVVIT.EventModel(cookie.model);new_model.set({last_update:new_model.get("startDate"),histogram:[]});self.model.add(new_model);self.render();break}}$.cookie("com.sivvit.event",null)}})},deleteItem:function(itm){itm.html.fadeOut();this.model.remove(itm.model,{silent:true})},updateItem:function(itm){itm.html.find("#title").html("<a href='"+SIVVIT.Settings.host+"/event/"+itm.model.get("id")+"'>"+itm.model.get("title")+
"</a>");itm.html.find("#description").html(itm.model.get("description"));this.toggleLive(itm)},toggleLive:function(itm){var flag=itm.html.find("#pending-flag");if(itm.model.get("status")===1){flag.toggleClass("idle-notice",false);flag.toggleClass("live-notice",true)}else{flag.toggleClass("idle-notice",true);flag.toggleClass("live-notice",false)}}})})($,SIVVIT);Date.prototype.format=function(){return this.getMonth()+1+"/"+this.getDate()+"/"+String(this.getFullYear()).substr(2,2)+" "+this.getHours()+":"+this.getMinutes()+":"+this.getSeconds()};Date.secondsToDate=function(seconds){return new Date(seconds*1E3)};Date.dateToSeconds=function(date){return Math.round(date.getTime()/1E3)};Date.plusSecond=function(date){if(date.getSeconds()===59)return Date.plusMinute(new Date(date.setSeconds(0)));else return new Date(date.setSeconds(date.getSeconds()+1))};
