if(typeof SIVVIT=="undefined")SIVVIT={};SIVVIT.Settings={host:"http://sivvit.com"};(function(jQuery,SIVVIT){SIVVIT.EditEvent={model:null,view:null,init:function(id){var self=this;if(id===null||id===undefined){this.model=new SIVVIT.EventModel({startDate:(new Date).getTime()/1E3,endDate:(new Date((new Date).getTime()+864E5)).getTime()/1E3,pull:false,meta:0,type:null});self.setLocation()}else this.model=new SIVVIT.EventModel({json:SIVVIT.Settings.host+"/event/"+id+".json?callback=?",pull:false,meta:0,type:null});this.view=new SIVVIT.EditEventView({model:this.model});if(id===null||
...
8));$("input[name='start-time']").val(this.model.get("startDate").toTimeString().substring(0,8));$("#collection-btn").html(this.model===0?"Start Collection":"Start Collection");this.validate()},validate:function(){$("#form-main").validate({"rules":{title:{required:true,minlength:4},"keywords":{required:true,minlength:3},"start-date":{required:true,date:true},"end-date":{required:true,date:true}}});var i,field,valid,icon;var global_valid=true;for(i=0;i<this.required_fields.length;i++){field=$(this.required_fields[i].field);
valid=this.validateValue(field.val(),this.required_fields[i].type);global_valid=valid?global_valid:false;icon=$(this.required_fields[i].icon);field.css("background-color",valid?"#FFFFCC":"#FFFFFF");icon.toggleClass("icon-check-green",valid?false:true);icon.toggleClass("icon-check-red",valid?true:false)}field=$("input[name='start-time']");valid=this.validateTime(field.val());global_valid=valid?global_valid:false;field.css("background-color",valid?"#FFFFFF":"#FFFFCC");$("#icon-start-time").toggleClass("icon-check-red",
