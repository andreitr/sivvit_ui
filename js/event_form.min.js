...
Backbone.View.extend({el:"#form-container",map:null,map_complete:null,events:{"click #save-event-btn":"saveEvent","click #delete-event-btn":"deleteEvent"},initialize:function(options){this.model=options.model;this.model.bind("change",this.update,this);this.model.bind("change:location",this.initMap,this);this.setValidationRules()},deleteEvent:function(){},saveEvent:function(){if(this.model.get("id"))this.model.updateEvent();else this.model.createEvent()},update:function(){var self=this;$("#form-container").show();
...
{silent:true})});$("input[name='description']").val(this.model.get("description"));$("input[name='description']").change(function(){self.model.set({"description":$(this).val()},{silent:true})});$("input[name='start-date']").val(this.model.get("startDate").format());$("input[name='start-date']").datetimepicker({dateFormat:"mm/dd/yy",minDate:new Date,maxDate:this.model.get("endDate"),defaultDate:this.model.get("startDate"),hour:this.model.get("startDate").getHours(),minute:this.model.get("startDate").getMinutes(),
second:this.model.get("startDate").getSeconds(),onSelect:function(date){self.model.set({"startDate":Date.dateToSeconds(new Date(date))},{silent:true})}});$("input[name='end-date']").val(this.model.get("endDate").format());$("input[name='end-date']").datetimepicker({dateFormat:"mm/dd/yy",defaultDate:this.model.get("endDate"),minDate:this.model.get("starDate"),hour:this.model.get("endDate").getHours(),minute:this.model.get("endDate").getMinutes(),second:this.model.get("endDate").getSeconds(),onSelect:function(date){self.model.set({"endDate":Date.dateToSeconds(new Date(date))},
{silent:true})}});$("#form-main").validate().element("input[name='title']");$("#form-main").validate().element("input[name='keywords']");$("#form-main").validate().element("input[name='start-date']");$("#form-main").validate().element("input[name='end-date']")},setValidationRules:function(){var self=this;$("#form-main").validate({success:function(label){},errorPlacement:function(error,element){},highlight:function(element,errorClass){var check=$(self.el).find("#"+$(element).attr("name")+"-check");
check.toggleClass("icon-check-red",true);check.toggleClass("icon-check-green",false);$(element).css("background-color","#FFFFCC")},unhighlight:function(element,errorClass){var check=$(self.el).find("#"+$(element).attr("name")+"-check");check.toggleClass("icon-check-red",false);check.toggleClass("icon-check-green",true);$(element).css("background-color","#FFFFFF")},rules:{title:{required:true,minlength:3},keywords:{required:true,minlength:3},"start-date":{required:true,date:true},"end-date":{required:true,
date:true}}})},initMap:function(){var self=this;if(!this.map){this.map=new google.maps.Map($("#form-map")[0],{center:new google.maps.LatLng(self.model.get("location").lat,self.model.get("location").lon),zoom:8,disableDefaultUI:true,mapTypeId:google.maps.MapTypeId.ROADMAP});this.map_complete=new google.maps.places.Autocomplete($("input[name='location']")[0]);google.maps.event.addListener(this.map_complete,"place_changed",function(){var place=self.map_complete.getPlace();self.map.setCenter(place.geometry.location);
