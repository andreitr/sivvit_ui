...
Backbone.View.extend({el:"#form-container",map:null,map_complete:null,events:{"click #save-event-btn":"saveEvent","click #delete-event-btn":"deleteEvent"},initialize:function(options){var self=this;this.model=options.model;this.model.bind("change",this.update,this);this.model.bind("change:location",this.initMap,this)},deleteEvent:function(){},saveEvent:function(){if(this.model.get("id"))this.model.updateEvent();else this.model.createEvent()},update:function(){var slef=this;$("#form-container").show();
...
{silent:true})});$("input[name='description']").val(this.model.get("description"));$("input[name='description']").change(function(){slef.model.set({"description":$(this).val()},{silent:true})});$("input[name='start-date']").datepicker({defaultDate:this.model.get("startDate"),onSelect:function(date){slef.model.set({"startDate":(new Date(date)).getTime()/1E3},{silent:true})}});$("input[name='start-date']").val(this.model.get("startDate"));$("input[name='end-date']").datepicker({defaultDate:this.model.get("endDate"),
onSelect:function(date){slef.model.set({"endDate":(new Date(date)).getTime()/1E3},{silent:true})}});$("input[name='end-date']").val(this.model.get("endDate").toDateString());this.validate()},validate:function(){var self=this;$("#form-main").validate({success:function(label){},errorPlacement:function(error,element){},highlight:function(element,errorClass){var check=$(self.el).find("#"+$(element).attr("name")+"-check");check.addClass("icon-check-red");check.removeClass("icon-check-green");$(element).css("background-color",
"#FFFFCC")},unhighlight:function(element,errorClass){var check=$(self.el).find("#"+$(element).attr("name")+"-check");check.removeClass("icon-check-red");check.addClass("icon-check-green");$(element).css("background-color","#FFFFFF")},rules:{title:{required:true,minlength:3},keywords:{required:true,minlength:3},"start-date":{required:true,date:true},"end-date":{required:true,date:true},location:{required:true,minlength:3}}})},initMap:function(){var self=this;if(!this.map){this.map=new google.maps.Map($("#form-map")[0],
